...
-- Operador IN (base de datos jardinería)
-- 2) Selecciona aquellos nombres de clientes que hayan hecho algún pedido en 2008.
SELECT nombre_cliente FROM cliente 
...
IN (SELECT codigo_oficina FROM oficina WHERE ciudad = 'Madrid');
...
SELECT * FROM cliente WHERE codigo_cliente 
NOT IN (SELECT codigo_cliente FROM pago WHERE forma_pago = 'PayPal');

-- 6) Selecciona aquellos empleados que sean jefes.
SELECT * FROM empleado WHERE codigo_empleado 
IN (SELECT DISTINCT codigo_jefe FROM empleado WHERE codigo_jefe IS NOT NULL);
...
-- Otra forma de hacerlo con varias subconsultas sería:
SELECT * FROM empleado WHERE codigo_empleado 
IN (SELECT codigo_jefe FROM empleado WHERE codigo_jefe IS NOT NULL) 
and codigo_oficina IN (SELECT codigo_oficina FROM oficina WHERE pais LIKE 'España');

-- Operador ANY o SOME 
-- 9) Selecciona aquellos clientes cuyo límite de crédito sea superior, 
-- al menos, al de algunos de los clientes. 
SELECT * FROM Cliente
WHERE Limite_Credito > SOME (SELECT Limite_Credito FROM Cliente);

-- 10) Selecciona aquellos clientes que hayan hecho, al menos, 
-- 5 pedidos durante el año 2009. 
SELECT * FROM Cliente 
...
-- Sería equivalente la siguiente consulta usando el operador IN: 
SELECT * FROM Cliente WHERE Codigo_Cliente 
IN (SELECT Codigo_Cliente FROM Pedido WHERE YEAR(Fecha_Pedido)='2009' 
GROUP BY Codigo_Cliente HAVING count(*) >=5); 

-- 11) Mostrar aquellos empleados que sean representantes de ventas de 
-- algún cliente de Madrid. 
SELECT * FROM Empleado
...
-- OPERADOR ALL
-- 15) Mostrar aquellos clientes que tengan su límite de crédito mayor a todos
SELECT * FROM Cliente
...
-- tenga su código de oficina que termine en '-ES'. 
...
-- 18) Mostrar aquellos productos de los que se hayan pedido más de 50 unidades. 
